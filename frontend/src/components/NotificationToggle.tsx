  ...
  if (!supported) {
    return (
      <div className="flex items-center justify-between p-4 bg-slate-800 rounded-lg">
        ...
        <div className="w-12 h-6 bg-slate-700 rounded-full opacity-50" />
      </div>
    );
  }
  ...
  if (permission === 'denied') {
    return (
      <div className="flex items-center justify-between p-4 bg-slate-800 rounded-lg">
        <div>
          <p className="font-medium">Push Notifications</p>
          <p className="text-sm text-red-400">Blocked - Enable in browser settings</p>
        </div>
        <div className="w-12 h-6 bg-slate-700 rounded-full opacity-50" />
      </div>
    );
    ...
    <div className="flex items-center justify-between p-4 bg-slate-800 rounded-lg">
      ...
      <button
        ...
        className={`relative w-12 h-6 rounded-full transition-colors ${
          subscribed ? 'bg-indigo-500' : 'bg-slate-600'
        } ${loading ? 'opacity-50' : ''}`}
      >
        <span
